...
{% for stack in stacks %}- **{{ stack.framework }}**{% if stack.version %} v{{ stack.version }}{% endif %}{% if stack.id != stack.framework %} (id: `{{ stack.id }}`){% endif %}{% if stack.package_manager %} · {{ stack.package_manager }}{% endif %}{% if stack.test_runner %} · {{ stack.test_runner }}{% endif %}
{% endfor %}

## Integrations

- Document tracker: `{{ integrations.doc_tracker }}`
- Issue tracker: `{{ integrations.issue_tracker }}`
{% if has_design %}- Design tool: `{{ integrations.design }}`
{% endif %}- VCS: `{{ integrations.vcs }}`
- CI: `{{ integrations.ci }}`
