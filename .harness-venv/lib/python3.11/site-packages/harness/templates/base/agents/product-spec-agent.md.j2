...
7. Hand off to `ux-agent` (provider: `{{ integrations.design }}`). The ux-agent reads `REQUIREMENTS.md § Screen inventory` and writes `DESIGN.md` + replaces your `## Visual spec` stub with the final one-line pointer + sets `design = complete` in `docs/features/$ARGUMENTS/state.json` (B-tier; mirror to index).
{% else %}
...
Story:      $ARGUMENTS
Test cases: <N>.
Visual spec: {% if integrations.design != "none" %}pending — ux-agent ({{ integrations.design }}) next{% else %}n/a (no design integration){% endif %}.
Next: {% if integrations.design != "none" %}invoke ux-agent then{% endif %} /arh-plan-implementation $ARGUMENTS
```
