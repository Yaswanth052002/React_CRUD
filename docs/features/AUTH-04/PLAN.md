...
| F-05 | create | `frontend/src/__tests__/App.auth.test.jsx`                      | Integration coverage for TC-01, TC-02, TC-03, TC-05, TC-10, TC-13, TC-14 (mount restoration, no-token render, expiry, malformed token, prop threading, mount-path logging, aria-live DOM check) |
...
| T-05  | Integration tests: `App.auth.test.jsx`                                                            | M          |     | T-03          | F-05  | Covers TC-01, TC-02, TC-03, TC-05, TC-10 (asserts a `sessionExpiredMessage` prop-equivalent value and the absence of any `authError` value passed downstream), TC-13 (mount-expiry-path log assertion), TC-14 (DOM query under Vitest/jsdom for an `aria-live` region containing the message text — declared `type: e2e` in the TC JSON but executed under the already-configured Vitest/jsdom runner, not a browser-automation tool, per the Runner-setup rationale in § 7) |
...
| Integration                    | `frontend/src/__tests__/App.auth.test.jsx`                      | TC-01, TC-02, TC-03, TC-05, TC-10                | Vitest + React Testing Library; seeds `localStorage` before render to simulate refresh/first-visit/expired/malformed scenarios |
...
| Accessibility (declared `e2e`)  | `frontend/src/__tests__/App.auth.test.jsx`                      | TC-14                                            | Declared `type: e2e` in `docs/test-cases/AUTH-04.json`, but executed under the existing Vitest/jsdom runner as a DOM query for an `aria-live` region containing the `sessionExpiredMessage` text — this repo has no browser-automation e2e runner (`docs/config/project-commands.yaml test_e2e: n/a`) and this story introduces none, per the same rationale AUTH-01's PLAN.md applied to its own e2e-typed TCs |
...
Every TC in `docs/test-cases/AUTH-04.json` (TC-01 through TC-15) appears in the table above; none are flagged `manual: true`. Coverage gate: frontend unit/integration coverage follows the repo's existing threshold (no `harness.yaml` override present -> 80% default, per `docs/config/project-commands.yaml`). No e2e or contract runner is installed or configured by this story — TC-14 and TC-15 execute under the already-configured Vitest runner per the rationale in their respective rows above, consistent with AUTH-01/AUTH-02/AUTH-03's precedent for declared-but-runner-less TC types in this repo. Performance test (T-06/F-06) runs as part of the default `npm run test` invocation (no `perf`-label gating is declared for this story, unlike AUTH-02's backend load-test harness, because this is a fast in-process DOM benchmark, not a sustained-load test).
